      ...
      <div role="button" tabIndex={0} onClick={() => setOpen((o) => !o)}
        ...
        {selectedStore ? (
          <>
            ...
            <span className="text-white flex-1 truncate">{selectedStore.name}</span>
            <button type="button" onClick={handleClear} className="text-text-secondary hover:text-white transition-colors p-0.5"><Icon name="close" size={16} /></button>
          </>
        ) : (
          <>
            <Icon name="store" className="text-text-secondary" size={18} />
            <span className="text-text-secondary flex-1">Select a store...</span>
          </>
        )}
        ...
        <div className="absolute top-full left-0 right-0 mt-1 z-50 rounded-xl border border-border-dark bg-surface-dark shadow-2xl overflow-hidden">
          <div className="p-2 border-b border-border-dark">
            <div className="relative">
              <Icon name="search" size={18} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-text-secondary" />
              <input ref={searchRef} type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search stores..."
                className="w-full rounded-lg bg-background-dark border border-border-dark py-2 pl-8 pr-3 text-sm placeholder-text-secondary text-white focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none transition-all" />
            </div>
          </div>
          <div className="max-h-[220px] overflow-y-auto py-1">
            {isLoading ? (<div className="px-4 py-6 text-center text-text-secondary text-sm">Loading stores...</div>
            ) : filtered.length === 0 ? (<div className="px-4 py-6 text-center text-text-secondary text-sm">{search ? 'No stores match your search' : 'No stores yet'}</div>
            ) : (filtered.map((store) => (
                <button key={store.id} type="button" onClick={() => handleSelect(store.id)}
                  className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition-colors ${value === store.id ? 'bg-primary/15 text-primary font-semibold' : 'text-white hover:bg-background-dark'}`}>
                  <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: store.color }} />
                  <span className="flex-1 truncate">{store.name}</span>
                  {store.location && <span className="text-xs text-text-secondary truncate max-w-[120px]">{store.location}</span>}
          ...
          {!showNewForm ? (
            <button type="button" onClick={() => setShowNewForm(true)}
              className="w-full flex items-center gap-3 px-4 py-3 text-sm text-primary hover:bg-background-dark transition-colors">
              <Icon name="add_circle" size={18} /><span className="font-medium">Add new store</span>
            </button>
          ) : (
            <div className="p-3 space-y-3">
              <div className="text-xs font-medium text-text-secondary uppercase tracking-wider">New Store</div>
              <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Store name" autoFocus
                ...
                className="w-full rounded-lg bg-background-dark border border-border-dark py-2 px-3 text-sm placeholder-text-secondary text-white focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none transition-all" />
              <div>
                <div className="text-xs text-text-secondary mb-1.5">Color</div>
                <div className="flex flex-wrap gap-1.5">
                  {PRESET_COLORS.map((color) => (
                    <button key={color} type="button" onClick={() => setNewColor(color)}
                      className={`w-6 h-6 rounded-full transition-all ${newColor === color ? 'ring-2 ring-white ring-offset-2 ring-offset-surface-dark scale-110' : 'hover:scale-110'}`}
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => { setShowNewForm(false); setNewName(''); }}
                  className="flex-1 py-2 rounded-lg text-sm font-medium text-text-secondary hover:text-white hover:bg-background-dark transition-colors">Cancel</button>
                <button type="button" onClick={handleCreate} disabled={!newName.trim() || isCreating}
                  className="flex-1 py-2 rounded-lg text-sm font-bold bg-primary hover:bg-emerald-400 text-background-dark transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
