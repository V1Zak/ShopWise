...
export function StatCard({ label, value, trend, trendLabel, icon, iconColor = 'text-primary', children, highlight }: StatCardProps) {
  return (
    <div className={`relative overflow-hidden rounded-xl p-5 border shadow-sm group ${highlight ? 'bg-surface-dark border-primary/30 ring-1 ring-primary/20' : 'bg-surface-dark border-border-dark'}`}>
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-baseline gap-2 mb-2">
          <h3 className={`text-3xl font-bold ${highlight ? 'text-primary' : 'text-white'}`}>{value}</h3>
